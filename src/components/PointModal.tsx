...
export const PointModal: React.FC<PointModalProps> = ({
  ...
  useEffect(() => {
    if (initialPoint) {
      ...
      setBusinessLicenseImages(initialPoint.businessLicenseImages || []);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setValidationError(null);
    try {
      const compressed = await compressMultipleImages(e.target.files);
      if (compressed.length > 0) {
        setBusinessLicenseImages((prev) => [...prev, ...compressed.map((c) => c.dataUrl)]);
      }
    } catch (err: any) {
      setValidationError(err?.message || 'Không thể xử lý hình ảnh. Vui lòng thử lại.');
    } finally {
  ...
  const handleRemoveImage = (indexToRemove: number) => {
    setBusinessLicenseImages((prev) => prev.filter((_, idx) => idx !== indexToRemove));
  };
      ...
      >
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden min-h-0 bg-slate-900 relative h-full">
          ...
          <form 
            ...
            } w-full md:w-[320px] lg:w-[340px] xl:w-[360px] bg-white border-r border-slate-200 flex-col shrink-0 h-full min-h-0 z-10 shadow-sm transition-all duration-200 overflow-y-auto`}
          >
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-2 space-y-1.5">
              ...
              <div className="space-y-1.5 pt-1.5 border-t border-slate-200">
                <div className="flex items-center justify-between">
                  ...
                  {businessLicenseImages.length > 0 && (
                    <span className="text-[9px] font-bold text-indigo-700 bg-indigo-50 border border-indigo-200 px-1.5 py-0.5 rounded-full">
                      {businessLicenseImages.length} ảnh
                    </span>
                  )}
                </div>

                <p className="text-[10px] text-slate-500 leading-tight">
                  Lưu ảnh giấy phép ĐKKD của chủ sở hữu. Hỗ trợ chụp từ điện thoại/tablet hoặc chọn từ máy tính.
                </p>

                {/* Hidden File Input supporting multiple images & camera */}
                <input
                  id="input-point-business-license"
                  type="file"
                  multiple
                  accept="image/*"
                ...
                <label
                  htmlFor="input-point-business-license"
                  className={`w-full py-2 px-2.5 rounded-lg border-2 border-dashed transition-all flex flex-col items-center justify-center gap-1 cursor-pointer select-none ${
                    isCompressingImages 
                      ? 'bg-slate-50 border-slate-300 pointer-events-none opacity-60' 
                      : 'bg-indigo-50/50 hover:bg-indigo-50 border-indigo-200 hover:border-indigo-400 text-indigo-700'
                  }`}
                >
                  {isCompressingImages ? (
                    <div className="flex items-center gap-1.5 text-xs text-indigo-600 font-semibold py-1">
                    ...
                    <>
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs font-bold text-indigo-900">
                          {businessLicenseImages.length === 0 ? 'Chọn ảnh / Chụp ảnh Giấy ĐKKD' : 'Thêm ảnh Giấy ĐKKD'}
                        </span>
                      </div>
                      <span className="text-[9px] text-slate-500 font-medium">
                        Chọn được nhiều ảnh cùng lúc (PC, Mobile, Tablet)
                      </span>
                    </>
                  )}
                </label>
                ...
                {businessLicenseImages.length > 0 && (
                  <div className="space-y-1.5 pt-1">
                    <div className="grid grid-cols-3 gap-1.5">
                      {businessLicenseImages.map((imgUrl, idx) => (
                        <div
                          key={idx}
                          className="group relative aspect-square rounded-lg border border-slate-200 overflow-hidden bg-slate-100 shadow-2xs hover:border-indigo-400 transition-all"
                        >
                          <img
                            src={imgUrl}
                            alt={`Giấy phép ${idx + 1}`}
                          ...
                          />
                          <span className="absolute top-1 left-1 text-[9px] font-bold bg-black/75 text-white px-1 rounded backdrop-blur-xs pointer-events-none">
                            #{idx + 1}
                          </span>
                          ...
                          <div className="absolute inset-0 bg-slate-950/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1.5 p-1">
                            <button
                              ...
                              className="p-1 rounded bg-white/95 hover:bg-white text-slate-800 transition-colors cursor-pointer"
                      ...
                      <label
                        htmlFor="input-point-business-license"
                        className="aspect-square rounded-lg border border-dashed border-indigo-300 hover:border-indigo-500 bg-indigo-50/50 hover:bg-indigo-100/70 transition-colors flex flex-col items-center justify-center gap-0.5 cursor-pointer text-indigo-600"
                        title="Thêm ảnh giấy phép khác"
                      >
                    ...
                    <div className="flex items-center justify-between text-[10px] text-slate-500 pt-0.5">
                      <span>Nhấp vào ảnh để xem toàn màn hình</span>
                      <button
                        type="button"
                        onClick={() => setBusinessLicenseImages([])}
        ...
        {/* Lightbox Viewer for Business Registration Certificate */}
        <ImageLightboxModal
          isOpen={lightboxOpen}
          onClose={() => setLightboxOpen(false)}
          images={businessLicenseImages}
          initialIndex={lightboxIndex}
          title={`Giấy ĐKKD - ${owner || name || 'Chủ cơ sở'}`}
          subtitle={name}
        />
      </div>
    </div>
